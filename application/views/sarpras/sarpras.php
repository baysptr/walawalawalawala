...
<main role="main" class="flex-shrink-0"><br/>
	<div class="container">
		<table class="table">
			...
			<tbody>
			<?php $no = 1; foreach ($sarprass as $sarpras){ ?>
				<tr>
					<td><?= $no++ ?></td>
					<td><?= $sarpras['nama'] ?></td>
					<td><?= $sarpras['jumlah'] ?></td>
					<td><?= $sarpras['bobot'] ?></td>
					<td><div onclick="editUser('<?= $sarpras['id'] ?>')" class="btn btn-sm btn-block btn-warning"><span class="fa fa-user-edit"></span> Edit</div></td>
					<td><div onclick="hapusUser('<?= $sarpras['id'] ?>')" class="btn btn-sm btn-block btn-danger"><span class="fa fa-user-times"></span> Delete</div></td>
				</tr>
			<?php } ?>
			</tbody>
		</table>
...
<script>
	...
	function editUser(id) {
		$.ajax({
			url: "<?= site_url() ?>sarpras/get_sarpras/" + id,
			type: "GET",
			dataType: "JSON",
	...
	function insertUser() {
		$.ajax({
			url: '<?= site_url() ?>sarpras/do_sarpras',
			type: 'POST',
			processData: false,
			cache: false,
			contentType: false,
	...
	function hapusUser(id) {
		if(confirm("apakah anda yakin untuk menghapus bari data ini?")===true){
			$.ajax({
				url: "<?= site_url() ?>sarpras/hapus_sarpras/" + id,
				type: "GET",
				success: function (data)
				{
					window.location.reload();
